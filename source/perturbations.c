...
static void perturbations_gedf_sound_speeds(
  ...
  *cs2_gedf = (ppt->set_const_cs2 == _TRUE_) ? ppt->cs2_gedf : 1.;

  if (fabs(one_plus_w) > 1.e-10) {
    *ca2_gedf = w_gedf - a * dw_over_da_gedf / 3. / one_plus_w;
  }
  else if (pba->use_model_independent_gedf == _TRUE_) {
    /* A constant w=-1 interval has no propagating fluid perturbation. The
       input layer rejects a finite-redshift w=-1 endpoint when perturbations
       are enabled, but this branch keeps background-only runs regular. */
    *ca2_gedf = w_gedf;
  }
  else if (fabs(1. + pba->w_1) < 1.e-10) {
    /* At early times every exponential step is linear in a^q, so this
       is the finite analytic limit even with two transitions. */
    *ca2_gedf = pba->w_1 - pba->q_gedf / 3.;
  }
  else {
    *ca2_gedf = w_gedf;
  }
    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          /* EDF by yehuang */
          class_store_double(dataptr,tk[ppt->index_tp_delta_edf],ppt->has_source_delta_edf,storeidx);
          class_store_double(dataptr,tk[ppt->index_tp_delta_gedf],ppt->has_source_delta_gedf,storeidx);
        ...
        if (ppt->has_velocity_transfers == _TRUE_) {
          ...
          /* EDF by yehuang */
          // class_store_double(dataptr,tk[ppt->index_tp_theta_edf],ppt->has_source_theta_edf,storeidx);
          class_store_double(dataptr,tk[ppt->index_tp_u_edf],ppt->has_source_u_edf,storeidx);
          class_store_double(dataptr,tk[ppt->index_tp_u_gedf],ppt->has_source_u_gedf,storeidx);
...
int perturbations_init(
  ...
  /* by yehuang */
  double w_edf_ini, w_edf_0,dw_over_da_edf,integral_edf;
  ...
  if (pba->has_edf == _TRUE_) {
    ...
    /* check values of w_fld at initial time and today. Since 'a' in the code stands for 'a/a_0', its current value is 1 by definition */
    class_call(background_w_edf(pba, 0., &w_edf_ini, &dw_over_da_edf, &integral_edf), pba->error_message, ppt->error_message);
    class_call(background_w_edf(pba, 1.,   &w_edf_0, &dw_over_da_edf, &integral_edf), pba->error_message, ppt->error_message);

    // class_test(w_fld_ini >= 0.,
    //            ppt->error_message,
    //            "The fluid is meant to be negligible at early time, and unimportant for defining the initial conditions of other species. You are using parameters for which this assumption may break down, since at early times you have w_fld(a--->0) = %e >= 0",w_fld_ini);
    // class_test((w_edf_0 == -1.) && (dw_over_da_edf == 0.),
    //              ppt->error_message,
    //              "Your choice of a EDF with w0=-1 is not valid due to instabilities in the unphysical perturbations of such a fluid. Try instead with a plain cosmological constant or with PPF scheme: use_ppf = yes");
  }
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        if (pba->has_edf == _TRUE_)
          // ppt->has_source_theta_edf = _TRUE_;
          ppt->has_source_u_edf = _TRUE_;
        if (pba->has_gedf == _TRUE_)
...
int perturbations_prepare_k_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      class_store_columntitle(ppt->scalar_titles,"delta_dadb",pba->has_dadb_d);
      class_store_columntitle(ppt->scalar_titles,"theta_dadb",pba->has_dadb_d);
      ...
      /** EDF by yehuang */
      class_store_columntitle(ppt->scalar_titles, "delta_rho_edf", pba->has_edf);
      class_store_columntitle(ppt->scalar_titles, "rho_plus_p_theta_edf", pba->has_edf);
      class_store_columntitle(ppt->scalar_titles, "delta_p_edf", pba->has_edf);
      class_store_columntitle(ppt->scalar_titles, "delta_edf", pba->has_edf);
      class_store_columntitle(ppt->scalar_titles, "cs2_edf_output", pba->has_edf);
      class_store_columntitle(ppt->scalar_titles, "ca2_edf_output", pba->has_edf);
      /** GEDF */
      class_store_columntitle(ppt->scalar_titles, "delta_rho_gedf", pba->has_gedf);
      class_store_columntitle(ppt->scalar_titles, "rho_plus_p_theta_gedf", pba->has_gedf);
      class_store_columntitle(ppt->scalar_titles, "delta_p_gedf", pba->has_gedf);
      class_store_columntitle(ppt->scalar_titles, "delta_gedf", pba->has_gedf);
      class_store_columntitle(ppt->scalar_titles, "ca2_gedf_output", pba->has_gedf);
...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    /* DADB coupled CDM keeps its own velocity even in synchronous gauge;
       uncoupled CDM remains the species that fixes the gauge. */
    class_define_index(ppv->index_pt_delta_dadb,pba->has_dadb_d,index_pt,1);
    class_define_index(ppv->index_pt_theta_dadb,pba->has_dadb_d,index_pt,1);
    ...
    /* EDF by yehuang */
    class_define_index(ppv->index_pt_delta_edf,pba->has_edf,index_pt,1); /* fluid density */
    // class_define_index(ppv->index_pt_theta_edf,pba->has_edf,index_pt,1);
    class_define_index(ppv->index_pt_u_edf,pba->has_edf,index_pt,1);
    // class_define_index(ppv->index_pt_Gamma_edf,pba->has_edf,index_pt,1);
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pba->has_dadb_d == _TRUE_) {
        ppv->y[ppv->index_pt_delta_dadb] =
          ppw->pv->y[ppw->pv->index_pt_delta_dadb];
        ppv->y[ppv->index_pt_theta_dadb] =
          ppw->pv->y[ppw->pv->index_pt_theta_dadb];
      }
      ...
      if (pba->has_edf == _TRUE_) {
          ...
          ppv->y[ppv->index_pt_delta_edf] =
            ppw->pv->y[ppw->pv->index_pt_delta_edf];

          // ppv->y[ppv->index_pt_theta_edf] =
          //   ppw->pv->y[ppw->pv->index_pt_theta_edf];

          ppv->y[ppv->index_pt_u_edf] =
            ppw->pv->y[ppw->pv->index_pt_u_edf];

          // ppv->y[ppv->index_pt_Gamma_edf] =
          //   ppw->pv->y[ppw->pv->index_pt_Gamma_edf];

      }

      if (pba->has_gedf == _TRUE_) {

          ppv->y[ppv->index_pt_delta_gedf] =
            ppw->pv->y[ppw->pv->index_pt_delta_gedf];
...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  double ac_edf, a_over_ac, ca2_edf, w_edf_f;
  double n_axion = 3;
  double cs2_edf_pheno;
  double cs2_gedf, ca2_gedf;
  double w_prime_gedf;
  ...
  if (pba->has_dadb_d == _TRUE_) {
    rho_m += ppw->pvecback[pba->index_bg_rho_dadb_d];
  }
  ...
  if (pba->has_gedf == _TRUE_) {
    class_call(background_w_gedf(pba, ppw->pvecback[pba->index_bg_a], &w_gedf, &dw_over_da_gedf, &integral_gedf),
               pba->error_message,
               ppt->error_message);
    rho_r += 3. * w_gedf * ppw->pvecback[pba->index_bg_rho_gedf];
    rho_m += ppw->pvecback[pba->index_bg_rho_gedf] - 3. * w_gedf * ppw->pvecback[pba->index_bg_rho_gedf];
  ...
  if (_scalars_) {
    ...
    if (pba->has_dadb_d == _TRUE_)
      fraccdm += ppw->pvecback[pba->index_bg_rho_dadb_d]/rho_m;
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_gedf == _TRUE_) {
        ...
        perturbations_gedf_sound_speeds(pba,ppt,a,w_gedf,dw_over_da_gedf,&ca2_gedf,&cs2_gedf);

          // ppw->pv->y[ppw->pv->index_pt_delta_gedf] = - ktau_two/4.*(1.+w_gedf)*(4.-3.*pba->cs2_edf)/(4.-6.*w_gedf+3.*pba->cs2_edf) * ppr->curvature_ini * s2_squared; /* from 1004.5509 */ //TBC: curvature
          // ppw->pv->y[ppw->pv->index_pt_delta_gedf] = - ktau_two/4.*(1.+w_gedf)*(4.-3.*cs2_gedf)/(4.-6.*w_gedf+3.*cs2_gedf)* ppr->curvature_ini * s2_squared;
          // ppw->pv->y[ppw->pv->index_pt_delta_gedf] = - 1e-3;

          // ppw->pv->y[ppw->pv->index_pt_theta_gedf] = - k*ktau_three/4.*pba->cs2_edf/(4.-6.*w_gedf+3.*pba->cs2_edf) * ppr->curvature_ini * s2_squared; /* from 1004.5509 */ //TBC:curvature

          // ppw->pv->y[ppw->pv->index_pt_u_gedf] = - k*ktau_three/4.*(1.+w_gedf)*pba->cs2_edf/(4.-6.*w_gedf+3.*pba->cs2_edf) * ppr->curvature_ini * s2_squared;
          // ppw->pv->y[ppw->pv->index_pt_u_gedf] = - k*ktau_three/4.*(1.+w_gedf)*cs2_gedf/(4.-6.*w_gedf+3.*cs2_gedf)* ppr->curvature_ini * s2_squared;
          // ppw->pv->y[ppw->pv->index_pt_u_gedf] = 0;

          ppw->pv->y[ppw->pv->index_pt_delta_gedf] = - 0.5*ktau_two*(1.+w_gedf)*(-4.+3.*cs2_gedf)/(32.+12.*w_gedf+6.*cs2_gedf)* ppr->curvature_ini * s2_squared;
          ppw->pv->y[ppw->pv->index_pt_u_gedf] = - 0.5*k*ktau_three*(1.+w_gedf)*cs2_gedf/(32.+12.*w_gedf+6.*cs2_gedf)* ppr->curvature_ini * s2_squared;

          if (ppt->has_gedf_perturbations == _FALSE_){
            ppw->pv->y[ppw->pv->index_pt_delta_gedf] = 0.;
            ppw->pv->y[ppw->pv->index_pt_u_gedf] = 0.;
          }
      ...
      if (pba->has_edf == _TRUE_) {
        ...
        // double omega_axion = 0; 
        // /* Be careful here we set omega_axion to 0.*/

        // ac_edf = 1./(1.+pba->zc_edf);
        // a_over_ac = a/ac_edf;
        // ca2_edf = (pow(a,3)*pow(a_over_ac,3*n_axion/(1+n_axion))*(-1+n_axion)-pow(a_over_ac,3/(1+n_axion))*pow(ac_edf,3)*(1+3*n_axion))
        //       /(pow(a,3)*pow(a_over_ac,3*n_axion/(1+n_axion))+pow(a_over_ac,3/(1+n_axion))*pow(ac_edf,3))/(1+n_axion);
        // w_edf_f = (n_axion-1)/(n_axion+1);
        // cs2_edf = (2*a*a*(n_axion-1)*pow(omega_axion*pow(a,-3*(n_axion-1)/(n_axion+1)),2)+k*k)/(2*a*a*(n_axion+1)*pow(omega_axion*pow(a,-3*(n_axion-1)/(n_axion+1)),2)+k*k);

        // ppw->pv->y[ppw->pv->index_pt_delta_edf] = 0.5*ktau_two*(1.+ppw->pvecback[pba->index_bg_w_edf])*(-4.+3.*cs2_edf)/(32.+6.*cs2_edf+12.*w_edf_f)* ppr->curvature_ini * s2_squared;

        // ppw->pv->y[ppw->pv->index_pt_theta_edf] = -0.5*k*ktau_three*cs2_edf/(32.+6.*cs2_edf+12.*w_edf_f)* ppr->curvature_ini * s2_squared;
        ...
        // cs2_edf_pheno = (2*a*a*(n_axion-1)*pow(pba->omega_axion*pow(a,-3*(n_axion-1)/(n_axion+1)),2)+k*k)/(2*a*a*(n_axion+1)*pow(pba->omega_axion*pow(a,-3*(n_axion-1)/(n_axion+1)),2)+k*k);
        // cs2_edf_pheno = 1.;
        // printf(" -> cs2_edf_pheno = %-15g \n", cs2_edf_pheno);
        ...
        if (w_edf == -1.){
          ...
          // ca2_edf = w_edf - w_prime_edf / 3. / (1.+w_edf) / a_prime_over_a;
          cs2_edf_pheno = (2*a*a*(n_axion-1)*pow(pba->omega_axion*pow(a,-3*(n_axion-1)/(n_axion+1)),2)+k*k)/(2*a*a*(n_axion+1)*pow(pba->omega_axion*pow(a,-3*(n_axion-1)/(n_axion+1)),2)+k*k);
        }

        ppw->pv->y[ppw->pv->index_pt_delta_edf] = - ktau_two/4.*(1.+w_edf)*(4.-3.*cs2_edf_pheno)/(4.-6.*w_edf+3.*cs2_edf_pheno) * ppr->curvature_ini * s2_squared; /* from 1004.5509 */ //TBC: curvature

        // ppw->pv->y[ppw->pv->index_pt_theta_edf] = - k*ktau_three/4.*cs2_edf_pheno/(4.-6.*w_edf+3.*cs2_edf_pheno) * ppr->curvature_ini * s2_squared; /* from 1004.5509 */ //TBC:curvature

        ppw->pv->y[ppw->pv->index_pt_u_edf] = - k*ktau_three/4.*(1.+w_edf)*cs2_edf_pheno/(4.-6.*w_edf+3.*cs2_edf_pheno) * ppr->curvature_ini * s2_squared;
        
        // Set to 0. for test.
        // ppw->pv->y[ppw->pv->index_pt_delta_edf] = 0.;
        // ppw->pv->y[ppw->pv->index_pt_u_edf] = 0.;
        /* if use_ppf == _TRUE_, y[ppw->pv->index_pt_Gamma_fld] will be automatically set to zero, and this is what we want (although one could probably work out some small nonzero initial conditions: TODO) */
      }
    ...
    if (pba->has_dadb_d == _TRUE_) {
      /* All supported runs are synchronous. The mass transition is far
         outside the initial-condition epoch for the intended late branch,
         so the regular adiabatic pressureless solution is appropriate. */
      ppw->pv->y[ppw->pv->index_pt_delta_dadb] =
        ppw->pv->y[ppw->pv->index_pt_delta_cdm];
      ppw->pv->y[ppw->pv->index_pt_theta_dadb] = 0.;
    }
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if (pba->has_edf == _TRUE_) {
        ...
        ppw->pv->y[ppw->pv->index_pt_delta_edf] -= 3*(1.+w_edf)*a_prime_over_a*alpha;
        // ppw->pv->y[ppw->pv->index_pt_theta_edf] += k*k*alpha;
        ppw->pv->y[ppw->pv->index_pt_u_edf] += k*k*alpha*(1.+w_edf);
      }
...
int perturbations_total_stress_energy(
  ...
  double rho_edf, p_edf, rho_edf_prime, p_edf_prime;
  double X, Y, Z, X_prime, Y_prime, Z_prime;
  double Gamma_fld, S, S_prime, theta_t, theta_t_prime, rho_plus_p_theta_fld_prime;
  double Gamma_edf, rho_plus_p_theta_edf_prime;
  double delta_p_b_over_rho_b;

  double a_over_ac, cs2_edf, ac_edf;
  double cs2_edf_pheno;
  double n_axion = 3;
  double n_edf = 3, omega_axion = 0;
  ...
  if (_scalars_) {
    ...
    /* DADB coupled pressureless species. Intrinsic X is deliberately kept
       smooth in this phenomenological closure; its background (rho+p) still
       belongs in rho_plus_p_tot. */
    if (pba->has_dadb_d == _TRUE_) {
      ppw->delta_rho += ppw->pvecback[pba->index_bg_rho_dadb_d]
        *y[ppw->pv->index_pt_delta_dadb];
      ppw->rho_plus_p_theta += ppw->pvecback[pba->index_bg_rho_dadb_d]
        *y[ppw->pv->index_pt_theta_dadb];
      ppw->rho_plus_p_tot += ppw->pvecback[pba->index_bg_rho_dadb_d];
      if (ppt->has_source_delta_m == _TRUE_) {
        delta_rho_m += ppw->pvecback[pba->index_bg_rho_dadb_d]
          *y[ppw->pv->index_pt_delta_dadb];
        rho_m += ppw->pvecback[pba->index_bg_rho_dadb_d];
      }
      if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
        rho_plus_p_theta_m += ppw->pvecback[pba->index_bg_rho_dadb_d]
          *y[ppw->pv->index_pt_theta_dadb];
        rho_plus_p_m += ppw->pvecback[pba->index_bg_rho_dadb_d];
      }
    }
    if (pba->has_dadb == _TRUE_) {
      ppw->rho_plus_p_tot += (1.+ppw->pvecback[pba->index_bg_dadb_w_X])
        *ppw->pvecback[pba->index_bg_rho_dadb_X];
    }
    ...
    */
    ...
    if (pba->has_gedf == _TRUE_) {
      ...
      ppw->delta_rho_gedf = ppw->pvecback[pba->index_bg_rho_gedf]*y[ppw->pv->index_pt_delta_gedf];

      ppw->rho_plus_p_theta_gedf = ppw->pvecback[pba->index_bg_rho_gedf]*y[ppw->pv->index_pt_u_gedf];

      /** We must gauge transform the pressure perturbation from the fluid rest-frame to the gauge we are working in */
      ppw->delta_p_gedf = cs2_gedf * ppw->delta_rho_gedf + (cs2_gedf-ca2_gedf)*(3*a_prime_over_a*ppw->rho_plus_p_theta_gedf/k/k);
      ppw->delta_gedf = y[ppw->pv->index_pt_delta_gedf];
      ppw->ca2_gedf_output = ca2_gedf;
      ppw->cs2_gedf_output = cs2_gedf;
      ppw->u_gedf = y[ppw->pv->index_pt_u_gedf];

      ppw->delta_rho += ppw->delta_rho_gedf;
      ppw->rho_plus_p_theta += ppw->rho_plus_p_theta_gedf;
      ppw->delta_p += ppw->delta_p_gedf;
    ...
    if (pba->has_edf == _TRUE_) {
      ...
      class_call(background_w_edf(pba,a,&w_edf,&dw_over_da_edf,&integral_edf), pba->error_message, ppt->error_message);
      w_prime_edf = dw_over_da_edf * a_prime_over_a * a;
      ac_edf = 1./(1.+pba->zc_edf);

      // if (pba->use_ppf == _FALSE_) {
        ppw->delta_edf = y[ppw->pv->index_pt_delta_edf];
        ppw->delta_rho_edf = ppw->pvecback[pba->index_bg_rho_edf]*y[ppw->pv->index_pt_delta_edf];
        // ppw->rho_plus_p_theta_edf = (1.+w_edf)*ppw->pvecback[pba->index_bg_rho_edf]*y[ppw->pv->index_pt_theta_edf];
        ppw->rho_plus_p_theta_edf = ppw->pvecback[pba->index_bg_rho_edf]*y[ppw->pv->index_pt_u_edf];
        // if (a < ac_edf){
        if (w_edf == -1.){
          ca2_edf = -5./2.;
          cs2_edf_pheno = 1.;
        ...
        // cs2_edf_pheno = (2*a*a*(n_axion-1)*pow(pba->omega_axion*pow(a,-3*(n_axion-1)/(n_axion+1)),2)+k*k)/(2*a*a*(n_axion+1)*pow(pba->omega_axion*pow(a,-3*(n_axion-1)/(n_axion+1)),2)+k*k);
        /** We must gauge transform the pressure perturbation from the fluid rest-frame to the gauge we are working in */
        ppw->cs2_edf_output = cs2_edf_pheno;
        ppw->ca2_edf_output = ca2_edf;
        ppw->delta_p_edf = cs2_edf_pheno * ppw->delta_rho_edf + (cs2_edf_pheno-ca2_edf)*(3*a_prime_over_a*ppw->rho_plus_p_theta_edf/k/k);
      // }
      ...
      //   a_over_ac = a/ac_edf;
      //   ca2_scf = -4*pow(a_over_ac,3/2)*pow(ac_edf,3)
      //         /(pow(a,3)*pow(a_over_ac,3/2)+pow(a_over_ac,3/2)*pow(ac_edf,3))/2;
      // }
      // else{}
      ...
      // c_gamma_k_H_square = pow(0.4*k/a_prime_over_a,2)*pba->cs2_edf;
      // // pba->c_gamma_over_c_edf
      ...
      //   Gamma_edf = 0.;
      // else
      //   Gamma_edf = y[ppw->pv->index_pt_Gamma_edf];

      // if (ppt->gauge == synchronous){
      //   alpha = (y[ppw->pv->index_pt_eta]+1.5*a2/k2/s2sq*(ppw->delta_rho+3*a_prime_over_a/k2*ppw->rho_plus_p_theta)-Gamma_edf)/a_prime_over_a;
      ...
      // }
      // ppw->S_edf = ppw->pvecback[pba->index_bg_rho_edf]*(1.+w_edf)*1.5*a2/k2/a_prime_over_a*
      ...
      // if (c_gamma_k_H_square > ppr->c_gamma_k_H_square_max){
      //   ppw->Gamma_prime_edf = 0.;
      // }
      // else{
      //   ppw->Gamma_prime_edf = a_prime_over_a*(ppw->S_edf/(1.+c_gamma_k_H_square) - (1.+c_gamma_k_H_square)*Gamma_edf);
      // }
      // Gamma_prime_plus_a_prime_over_a_Gamma = ppw->Gamma_prime_edf+a_prime_over_a*Gamma_edf;
      // // delta and theta in both gauges gauge:
      // ppw->rho_plus_p_theta_edf = ppw->pvecback[pba->index_bg_rho_edf]*(1.+w_edf)*ppw->rho_plus_p_theta/ppw->rho_plus_p_tot-
      //   k2*2./3.*a_prime_over_a/a2/(1+4.5*a2/k2/s2sq*ppw->rho_plus_p_tot)*
      //   (ppw->S_edf-Gamma_prime_plus_a_prime_over_a_Gamma/a_prime_over_a);
      // ppw->delta_rho_edf = -2./3.*k2*s2sq/a2*Gamma_edf-3*a_prime_over_a/k2*ppw->rho_plus_p_theta_edf;
      ...
      // rho_edf = ppw->pvecback[pba->index_bg_rho_edf];
      // p_edf = w_edf*rho_edf;
      // rho_edf_prime = -3*a_prime_over_a*(rho_edf+p_edf);
      // p_edf_prime = w_prime_edf*rho_edf-3*a_prime_over_a*(1+w_edf)*p_edf;
      // rho_t = ppw->pvecback[pba->index_bg_rho_tot] - rho_edf;
      // p_t = ppw->pvecback[pba->index_bg_p_tot] - p_edf;
      // rho_t_prime = -3*a_prime_over_a*(rho_t+p_t);
      // p_t_prime = ppw->pvecback[pba->index_bg_p_tot_prime]-p_edf_prime;
      ...
      // S = ppw->S_edf;
      // S_prime = -Z_prime/Z*S+1./Z*(rho_edf_prime+p_edf_prime)*(theta_t+k2*alpha)+1./Z*(rho_edf+p_edf)*(theta_t_prime+k2*alpha_prime);
      // /** Analytic derivative of the equation for ppw->rho_plus_p_theta_fld above. */
      // rho_plus_p_theta_edf_prime = Z_prime*(S-1./(1.+Y)*(S/(1.+1./X)+Gamma_edf*X)) +
      //   Z*(S_prime + Y_prime/(1.+Y*Y+2*Y)*(S/(1.+1./X)+Gamma_edf*X)-
      //       1./(1.+Y)*(S_prime/(1.+1./X)+S*X_prime/(1.+X*X+2*X)+ppw->Gamma_prime_edf*X+Gamma_edf*X_prime))-
      //   k2*alpha_prime*(rho_edf+p_edf)-k2*alpha*(rho_edf_prime+p_edf_prime);

      // /** We can finally compute the pressure perturbation using the Euler equation for theta_fld */
      // ppw->delta_p_edf = (rho_plus_p_theta_edf_prime+4*a_prime_over_a* ppw->rho_plus_p_theta_edf - (rho_edf+p_edf)*metric_euler)/k2;


      ppw->delta_rho += ppw->delta_rho_edf;
      ppw->rho_plus_p_theta += ppw->rho_plus_p_theta_edf;
      ppw->delta_p += ppw->delta_p_edf;

      ppw->rho_plus_p_tot += (1.+w_edf)*ppw->pvecback[pba->index_bg_rho_edf];
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    /* delta_edf */
    if (ppt->has_source_delta_edf == _TRUE_) {
      _set_source_(ppt->index_tp_delta_edf) = ppw->delta_rho_edf/pvecback[pba->index_bg_rho_edf]
        + 3.*a_prime_over_a*(1.+pvecback[pba->index_bg_w_edf])*theta_over_k2; // N-body gauge correction
    }

    /* delta_gedf has_gedf */ 
    if (ppt->has_source_delta_gedf == _TRUE_) {
      _set_source_(ppt->index_tp_delta_gedf) = ppw->delta_rho_gedf/pvecback[pba->index_bg_rho_gedf]
    ...
    /* theta_edf */
    // if (ppt->has_source_theta_edf == _TRUE_) {

    //   class_call(background_w_edf(pba,a,&w_edf,&dw_over_da_edf,&integral_edf), pba->error_message, ppt->error_message);

    //   _set_source_(ppt->index_tp_theta_edf) = ppw->rho_plus_p_theta_edf/(1.+w_edf)/pvecback[pba->index_bg_rho_edf]
    //     + theta_shift; // N-body gauge correction
    // }

    if (ppt->has_source_u_edf == _TRUE_) {

      class_call(background_w_edf(pba,a,&w_edf,&dw_over_da_edf,&integral_edf), pba->error_message, ppt->error_message);

      _set_source_(ppt->index_tp_u_edf) = ppw->rho_plus_p_theta_edf/pvecback[pba->index_bg_rho_edf]
        + theta_shift*(1.+w_edf); // N-body gauge correction
    }

    /** has_gedf */
    if (ppt->has_source_u_gedf == _TRUE_) {
...
int perturbations_print_variables(double tau,
  ...
  double delta_dadb=0.,theta_dadb=0.;
  ...
  if (_scalars_) {
    ...
    if (pba->has_dadb_d == _TRUE_) {
      delta_dadb = y[ppw->pv->index_pt_delta_dadb];
      theta_dadb = y[ppw->pv->index_pt_theta_dadb];
    }
    ...
    if (pba->has_edf == _TRUE_){
      ...
      if (w_edf == -1.){
        // ppw->ca2_edf_output = -5./2.;
        ppw->cs2_edf_output = 1.;
      }
      else{
        // ppw->ca2_edf_output = w_edf - w_prime_edf / 3. / (1.+w_edf) / a_prime_over_a;
        ppw->cs2_edf_output = (2*a*a*(n_axion-1.)*pow(pba->omega_axion*pow(a,-3*(n_axion-1.)/(n_axion+1.)),2)+k*k)/(2*a*a*(n_axion+1.)*pow(pba->omega_axion*pow(a,-3*(n_axion-1.)/(n_axion+1.)),2)+k*k);
      }
    }
    ...
    if (pba->has_gedf == _TRUE_){
      ...
      ppw->T_h_gedf = - (1. + w_gedf) * pvecmetric[ppw->index_mt_h_prime]/2.;

      delta_gedf = y[ppw->pv->index_pt_delta_gedf];
    ...
    if ((ppt->gauge == synchronous) && (ppt->get_perturbations_in_current_gauge == _FALSE_)) {
      ...
      if (pba->has_dadb_d == _TRUE_) {
        delta_dadb -= 3.*pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha;
        theta_dadb += k*k*alpha;
      }
    ...
    class_store_double(dataptr, delta_dadb, pba->has_dadb_d, storeidx);
    class_store_double(dataptr, theta_dadb, pba->has_dadb_d, storeidx);
    /* Interacting dark matter */
    ...
    //fprintf(ppw->perturbations_output_file,"\n");
    class_store_double(dataptr, ppw->delta_rho_edf, pba->has_edf, storeidx);
    class_store_double(dataptr, ppw->rho_plus_p_theta_edf, pba->has_edf, storeidx);
    class_store_double(dataptr, ppw->delta_p_edf, pba->has_edf, storeidx);
    class_store_double(dataptr, ppw->delta_edf, pba->has_edf, storeidx);
    class_store_double(dataptr, ppw->cs2_edf_output, pba->has_edf, storeidx);
    class_store_double(dataptr, ppw->ca2_edf_output, pba->has_edf, storeidx);
    /** GEDF */
    class_store_double(dataptr, ppw->delta_rho_gedf, pba->has_gedf, storeidx);
    class_store_double(dataptr, ppw->rho_plus_p_theta_gedf, pba->has_gedf, storeidx);
    class_store_double(dataptr, ppw->delta_p_gedf, pba->has_gedf, storeidx);
    class_store_double(dataptr, delta_gedf, pba->has_gedf, storeidx);
    class_store_double(dataptr, ppw->ca2_gedf_output, pba->has_gedf, storeidx);
...
int perturbations_derivs(double tau,
  ...
  double w_edf,dw_over_da_edf,w_prime_edf,integral_edf;
  double ac_edf;

  double w_gedf,dw_over_da_gedf,w_prime_gedf,integral_gedf;
  double cs2_gedf, ca2_gedf;
  ...
  double n_axion = 3;
  double dadb_qscale, dadb_W, dadb_mu5, dadb_kC, dadb_subhorizon;
  ...
  if (_scalars_) {
    ...
    if (pba->has_dadb_d == _TRUE_) {
      dy[pv->index_pt_delta_dadb] =
        -(y[pv->index_pt_theta_dadb]+metric_continuity);
      dy[pv->index_pt_theta_dadb] =
        -a_prime_over_a*(1.+pvecback[pba->index_bg_dadb_q])
        *y[pv->index_pt_theta_dadb];

      if (pba->dadb_mode == dadb_qs_yukawa && pba->dadb_beta5 > 0.) {
        dadb_qscale = 0.25*MAX(fabs(pba->dadb_Delta_e*pba->dadb_p_e),
                              fabs(pba->dadb_Delta_l*pba->dadb_p_l));
        if (dadb_qscale > 0.) {
          dadb_W = MIN(1.,pow(pvecback[pba->index_bg_dadb_q]/dadb_qscale,2));
          dadb_kC = pba->dadb_kC_hmpc*pba->h; /* h/Mpc -> 1/Mpc */
          dadb_subhorizon = k2/(k2+a_prime_over_a*a_prime_over_a);
          dadb_mu5 = 2.*pba->dadb_beta5*pba->dadb_beta5
            *k2/(k2+dadb_kC*dadb_kC)*dadb_W*dadb_subhorizon;
          /* With delta_dot=-theta/Hconf, a source S in theta' contributes
             -S/Hconf^2 to the ln(a) growth equation. Hence the minus sign
             below yields +3/2 mu5 Omega_d delta_d. This is an algebraic
             quasi-static closure, not a covariant microscopic scalar. */
          dy[pv->index_pt_theta_dadb] -= 1.5*a2
            *pvecback[pba->index_bg_rho_dadb_d]*dadb_mu5
            *y[pv->index_pt_delta_dadb];
        }
      }
    }
    ...
    if (pba->has_gedf == _TRUE_) {
        ...
        dy[pv->index_pt_delta_gedf] =
          -(y[pv->index_pt_u_gedf]+(1+w_gedf)*metric_continuity)
          -3.*(cs2_gedf-w_gedf)*a_prime_over_a*y[pv->index_pt_delta_gedf]
          -9.*(cs2_gedf-ca2_gedf)*a_prime_over_a*a_prime_over_a*y[pv->index_pt_u_gedf]/k2;

        // dy[pv->index_pt_delta_edf] =
        //   -(y[pv->index_pt_u_edf]+(1+w_edf)*metric_continuity)
        //   -3.*(cs2-w_edf)*a_prime_over_a*y[pv->index_pt_delta_edf]
        //   -9.*(cs2-ca2)*a_prime_over_a*a_prime_over_a*y[pv->index_pt_u_edf]/k2;

        /** - ----> fluid velocity */

        dy[pv->index_pt_u_gedf] = /* fluid velocity */
          -(1.-3.*cs2_gedf)*a_prime_over_a*y[pv->index_pt_u_gedf]
          +3*a_prime_over_a*(w_gedf - ca2_gedf)*y[pv->index_pt_u_gedf]
          +cs2_gedf*k2*y[pv->index_pt_delta_gedf]
          +metric_euler*(1.+w_gedf);

        // dy[pv->index_pt_u_edf] = /* fluid velocity */
        //   -(1.-3.*cs2)*a_prime_over_a*y[pv->index_pt_u_edf]
        //   +3*a_prime_over_a*(w_edf - ca2)*y[pv->index_pt_u_edf]
        //   +cs2*k2*y[pv->index_pt_delta_edf]
        //   +metric_euler*(1.+w_edf);

        // Turn off the perturbations.
    ...
    if (pba->has_edf == _TRUE_) {
        ...
        if (w_edf == -1.){
          ...
          ca2 = w_edf - w_prime_edf / 3. / (1.+w_edf) / a_prime_over_a;
          cs2 = (2*a*a*(n_axion-1)*pow(pba->omega_axion*pow(a,-3*(n_axion-1)/(n_axion+1)),2)+k*k)/(2*a*a*(n_axion+1)*pow(pba->omega_axion*pow(a,-3*(n_axion-1)/(n_axion+1)),2)+k*k);
        }
        // ca2 = w_edf - w_prime_edf / 3. / (1.+w_edf) / a_prime_over_a;
        // cs2 = pba->cs2_edf;
        ...
        // dy[pv->index_pt_delta_edf] =
        //   -(1+w_edf)*(y[pv->index_pt_theta_edf]+metric_continuity)
        //   -3.*(cs2-w_edf)*a_prime_over_a*y[pv->index_pt_delta_edf]
        //   -9.*(1+w_edf)*(cs2-ca2)*a_prime_over_a*a_prime_over_a*y[pv->index_pt_theta_edf]/k2;
        dy[pv->index_pt_delta_edf] =
          -(y[pv->index_pt_u_edf]+(1+w_edf)*metric_continuity)
          -3.*(cs2-w_edf)*a_prime_over_a*y[pv->index_pt_delta_edf]
          -9.*(cs2-ca2)*a_prime_over_a*a_prime_over_a*y[pv->index_pt_u_edf]/k2;
        ...
        // if (w_edf == -1.){
        //   dy[pv->index_pt_theta_edf] = 0.;
        // }
        // else{
        //   dy[pv->index_pt_theta_edf] = /* fluid velocity */
        //   -(1.-3.*cs2)*a_prime_over_a*y[pv->index_pt_theta_edf]
        //   +cs2*k2/(1.+w_edf)*y[pv->index_pt_delta_edf]
        //   +metric_euler;
        dy[pv->index_pt_u_edf] = /* fluid velocity */
          -(1.-3.*cs2)*a_prime_over_a*y[pv->index_pt_u_edf]
          +3*a_prime_over_a*(w_edf - ca2)*y[pv->index_pt_u_edf]
        ...
        // I do not know whether I need to modify the extra terms like metric_euler when I change theta_edf to u_edf.
        ...
        // dy[pv->index_pt_Gamma_edf] = ppw->Gamma_prime_edf;
